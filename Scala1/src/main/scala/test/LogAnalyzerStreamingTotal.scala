...
object LogAnalyzerStreamingTotal {
  ...
  //Seq is like list in java , List in scala is like linkedList in java.
  //Seq is implementation. But List is actual implementation
  //Option is container that can hold some value or non.
  // It s Like hashmap in java return null if not found
  //In option  case might be either spme value or none
  ...
  val runningCount = new AtomicLong(0)  // can be shared  by many threads
  ...
  def main(args: Array[String]) {
    ...
    // NOTE: Checkpointing must be enabled to use updateStateByKey.  //or reduceByKeyAndWindow
    ...
    val accessLogsDStream = logLinesDStream.map(AccessLogs.logParser).cache()
    ...
    endpointCountsDStream.foreachRDD(rdd => {
      val topEndpoints = rdd.top(10)(CustomOrdering.SecondValueLongSorting)
